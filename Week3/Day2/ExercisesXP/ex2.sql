/*
In the dvdrental database write a query to select all 
the columns from the “customer” table.
*/
...
/*
Write a query to get all the customer details 
from the customer table, it should be displayed 
...
/*
Write a query to retrieve all movie details where 
the movie id is either 15 or 150.
*/
...
/*
Write a query which should check if your favorite 
movie exists in the database. Have your query get 
the film ID, title, description, length and 
the rental rate, these details can be found in the 
“film” table.
*/

SELECT * FROM film
WHERE title = 'A Boy and His Dog';

-- case unsesitive
SELECT * FROM film
WHERE title ILIKE 'A Boy and His Dog';
...
/*
Not satisfied with the results. Write a query which
will find the next 10 cheapest movies.
...
name and last name from the curstomer table, as well 
...
SELECT film.title, film.description, film.release_year, inventory.last_update FROM film
LEFT JOIN inventory
ON film.film_id = inventory.film_id
WHERE inventory.last_update IS NULL;
...
/*
Bonus You want to be able to see how your sellers 
have been doing? Write a query to get the customer’s 
id, names (first and last), the amount and the date
of payment ordered by the id of the staff member who
sold them the dvd.
*/
