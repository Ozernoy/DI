...
conn.autocommit = True  # Enable autocommit mode so the CREATE DATABASE command is executed immediately
...
new_tb_name = 'Menu_Items'
cursor.execute("""CREATE TABLE {new_db_name}( 
                    item_id SERIAL PRIMARY KEY, 
                    item_name VARCHAR(30) NOT NULL, 
                    item_price SMALLINT DEFAULT 0)""")


cursor.execute("DROP DATABASE{new_db_name}")
cursor.close()
conn.close()
